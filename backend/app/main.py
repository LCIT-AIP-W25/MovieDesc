...
import shutil
import logging
from backend.app.audio_processing import tag_speakers_with_llm
...
from backend.app.video_processing import extract_frames
from backend.app.audio_processing import (
    extract_audio,
    transcribe_audio,
...
from backend.app.utils import ensure_directory, file_to_base64, cleanup_files
...
# --- FastAPI App ---
app = FastAPI(title="Movie Description API")
...
async def process_video(
    ...
    for f in os.listdir(TEMP_DIR):
        ...
        try:
            if os.path.isfile(file_path):
                os.remove(file_path)
                logger.info(f"[{request_id}] Deleted old file: {file_path}")
        except Exception as e:
            logger.warning(f"[{request_id}] Could not delete {file_path}: {e}")
    ...
    try:
        final_video_to_encode = None  # Prevent UnboundLocalError
        ...
        if audio_path_for_transcription and os.path.exists(audio_path_for_transcription):
            ...
            logger.info(f"[{request_id}] Tagging speakers using LLM...")
            tagged_transcript = tag_speakers_with_llm(transcription_text)
            from datetime import datetime

        # Define filename with timestamp
            # Cleanup old files in data/temp (keep only the latest ones)
            for f in os.listdir("data/temp"):
                file_path = os.path.join("data", "temp", f)
                try:
                    if os.path.isfile(file_path):
                        os.remove(file_path)
                        logger.info(f"[{request_id}] Deleted old temp file: {file_path}")
                except Exception as cleanup_err:
                    logger.warning(f"[{request_id}] Failed to delete {file_path}: {cleanup_err}")

            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            transcript_path = os.path.join("data", "temp", f"transcript_{timestamp}.txt")
            output_video_copy = os.path.join("data", "temp", f"output_{timestamp}.mp4")

            if not final_video_to_encode and os.path.exists(preprocessed_video_path):
                final_video_to_encode = preprocessed_video_path

            if final_video_to_encode and os.path.exists(final_video_to_encode):
                shutil.copy(final_video_to_encode, output_video_copy)
                logger.info(f"[{request_id}] Final video copied to: {output_video_copy}")
            else:
                logger.warning(f"[{request_id}] No final video to copy. Skipping video output save.")
        ...
        # Save tagged transcript
            ...
            logger.info(f"[{request_id}] Tagged transcript generated.")
